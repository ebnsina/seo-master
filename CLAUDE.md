...
| Deploy       | **`@sveltejs/adapter-node` + Docker Compose** (app + Postgres + Redis + Caddy) | `node build` runs the server _and_ the in-process worker, so a single VPS gets background jobs + schedulers. See `Dockerfile`, `docker-compose.yml`, `DEPLOY.md`. Runtime deps (incl. `drizzle-orm`, `postgres`) must stay in `dependencies`, not dev.   |
